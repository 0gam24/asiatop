...
- 2026-05-12 — auto-publish.yml 에 LLM 4-pass step 추가 (R54-2, V2 통합 2단계) — `dry_run='false'` 일 때만 article-pipeline.mjs --brief --draft 호출. DEEPSEEK_API_KEY + ANTHROPIC_API_KEY secrets 주입 (비용 ~$0.022/편). `_drafts/<slug>.mdx` 산출물 아티팩트 업로드. continue-on-error 안전망 (brief validation·LLM API 실패 시 step fail 만, 다음 작업 차단 X). Post-LLM 게이트 G5~G8 + PR 자동 생성은 R54-3 (다음 turn).
- 2026-05-12 — auto-brief-generator.mjs 의 짧은 TODO placeholder 4건 → 길이 통과 (R54-2 fix). 첫 dry_run=false dispatch 의 brief schema fail 4건 해결.
- 2026-05-12 — auto-publish.yml 에 Post-LLM 게이트 step 추가 (R54-3a, V2 통합 3단계). `dry_run='false'` 일 때 auto-publish.mjs --brief --mdx 호출, G4 fact-verify (중복 mock 캐시) + G5 adsense-policy + G6 disclosure-attach + G7 plagiarism + G8 ai-likeness 통합. 결과 JSON 아티팩트 업로드. PR 자동 생성은 R54-3b (운영자 검증 후).
...
- 2026-05-12 — R57: brief-prompt-builder LLM Pass 1 prompt 강화 — §10 sources 블록 헤더에 "G4 fact-verifier 매칭 풀" 명시 + amount/percent/count_unit/date/law 토큰 타입 규칙 + "약 N / 대략 N / 보통 N" approximate 우회 안내. §14 재진술에 "본문 사실 토큰 70% 이상 expected_facts 일치 의무" 명시. R55-3 의 expected_facts 자동 채움 + R57 prompt 강제 결합으로 G4 매칭률 22% → 70%+ 도달 목표.
