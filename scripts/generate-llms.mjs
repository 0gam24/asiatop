...
function renderLlmsTxt(articles, clusterStats) {
  ...
  out += `머니룩은 공공데이터포털(data.go.kr)·법제처 국가법령정보(law.go.kr)·홈택스·복지로·한국은행 ECOS 등 1차 정부·공공 자료를 기반으로 직장인이 매일 마주치는 돈 문제를 풀어 설명합니다. 모든 글은 편집팀이 1차 정부·공공 출처를 직접 확인해 작성·검수하며, 추정·변동 수치는 근사 표현으로 구분 표기합니다. 2026년 6월 이전 자동 발행분(AI 보조 공시 부착 글)은 발행 당시 10단계 자동 검증 게이트(G0~G9)와 정부 공식 API 사실 매칭(임계 70%)을 통과했습니다. 정책 변경 시 즉시 갱신됩니다.\n\n`;
