...
const PATTERNS = {
  ...
  percent: /(\d{1,3}(?:\.\d+)?)\s*%/g,
  // R59 — count_unit 단위 확장. LLM 본문에 자주 등장하지만 기존 PATTERN 미매칭이던 단위:
  //   영업일·평일·근무일 (실업급여·은행 처리 기간)
  //   주·분기·반기 (시간 단위)
  //   인·세대수·가구수 (인구 통계 — 가구·명 외)
  //   호·개·종·차 (차수·종류)
  count_unit: /(\d{1,3}(?:,\d{3})*)\s*(일|개월|년|세|회|건|명|가구|시간|분|영업일|평일|근무일|주|분기|반기|인|호|개|종|차|배|점)/g,
