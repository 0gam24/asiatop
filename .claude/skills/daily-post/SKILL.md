...
1. 수익 델타 1줄 (+정책 이슈 상태)
2. 발행 결과 — 편별 **선정 근거 1줄** (쉽게, 전문용어 최소). 링크는 여기 적지 않고 4번 블록에 모은다.
3. 다음 액션 1~2개 (예: 트리아지 진행률)
4. **발행 링크 블록 — 최종 메시지의 맨 마지막** (2026-09-05 운영자 지시: "맨 밑에 포스팅 링크").
   이 블록 뒤에는 어떤 문장도 붙이지 않는다. 자동 실행(예약·루틴)이든 수동 호출이든 동일.
   - 소제목 `발행 링크` 다음에 편당 한 줄: `- [신규] 글 제목 https://asiatop.co.kr/...` / `- [리프레시] 글 제목 https://asiatop.co.kr/...`
   - URL 200 + 내용 마커 확인이 끝난 편만 발행 링크로 적는다. 미완료 편(라벨 미부착·머지 대기·CF 빌드 중)은
     같은 자리에 PR 링크 + 상태 한 단어 (예: `- [신규·머지 대기] 글 제목 https://github.com/.../pull/N`)
   - 발행 0편인 날(중단·리프레시 없음)도 블록을 생략하지 않고 `발행 링크: 없음 (사유 한 줄)` 로 끝낸다.
