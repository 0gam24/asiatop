...
### 4-1. "오늘의 작업 정리" 블록 — 최종 메시지 **맨 아래 고정** (2026-08-28 운영자 지시)

사이클을 마치는 모든 메시지는 아래 블록으로 **끝나야** 한다. 중간에 끼워넣지 말고
항상 가장 하단에 둔다. 운영자가 이 블록만 읽고도 오늘 무슨 일이 있었는지 알 수 있어야 한다.

- **쉬운 말로**: 전문용어·내부 파일명·스크립트명·PR 번호를 본문에 늘어놓지 않는다.
  ("publish-cadence 가드 통과" ❌ → "발행 속도 규칙 통과" ✅)
- **편마다 링크 필수**: 신규·리프레시 가릴 것 없이 발행/수정한 글은 전부
  프로덕션 URL(`https://asiatop.co.kr/{cluster}/{slug}/`)을 함께 적는다. 링크 없는 항목 금지.
- **무엇이 바뀌었는지 1~3줄**: 특히 사실 오류를 고쳤다면 "무엇이 틀렸고 무엇으로 고쳤는지"를 먼저.
- **못 한 것/확인 필요**: URL 200 미확인, 차단된 네트워크, 보류한 판단 등은 숨기지 않고 마지막에.

형식 예시:

```
**오늘 작업 정리 (YYYY-MM-DD 데일리 포스팅)**

📝 수정한 글 (N편, 신규 M편)
1. <쉬운 제목>
   - <무엇을 고쳤는지 1~2줄>
   - 🔗 https://asiatop.co.kr/<cluster>/<slug>/
2. ...
