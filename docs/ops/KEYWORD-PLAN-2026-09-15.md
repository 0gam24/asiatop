# 네이버 빈틈 키워드 계획 v1 — 머니룩 (2026-09-15, 운영자 결재 완료)
...
- 1단계 프로파일: `SITE-KEYWORD-PROFILE.md` (축·트랙·패밀리·출처·함정·주제 범위·속도)
...
- **2026-09-15 운영자 결재 완료** (§13). 구현은 §10 순서로 진행한다.
...
| 2 | 롱테일은 더 관공서 | 롱테일 27개 문서 270건: 관공서 46% · 정보 사이트(상업·자매 도메인) 26% · 도구 9% · 공단 7% · 법률 4% · 금융사 4% | 열린 자리는 드물다. 자리를 재고 들어가야 한다 |
| 3 | **병목은 색인이 아니라 순위** | 9/8~9/15 루틴 신규 8편: 정확한 제목 검색으로 **7편 색인 확인**(1~6일). 대표 키워드로는 **7편 모두 30위 밖** | 사건 헤드("아동기본수당")가 아니라 **사건 × 대상** 롱테일을 타깃으로 |
| 4 | 정보 사이트가 1위인 롱테일이 있다 | 조기재취업수당 조건·주휴수당 15시간 미만은 **정보 사이트 1위**, 근로장려금 35% 선지급은 정보 사이트 1위·자사 5위 | 관공서가 아니라 **글로 이긴 자리**다. 글로 뺏을 수 있다는 증거 |
| 5 | 열린 자리의 절반은 **이미 우리 글이 있다** | 권고사직(3편)·주휴수당(2편)·조기재취업수당(04-04)·ISA 중도해지(07-03) — 모두 10위 밖 | 신규보다 **기존 글 갱신·진단**이 먼저인 자리가 많다 |
| 6 | 주제는 이미 범용이다 | 신규 8편이 고용·건강보험·종부세·운전면허·연금·복지·아동 7개 분야 | **범용 유지(2026-09-15 운영자 결정)**. 거르는 기준은 주제가 아니라 자리 |
...
1. **발행 트리거 = 열린 자리.** 위에 관공서·공단·도구·네이버 서비스가 적고, 빈자리가 있고, 우리 글이 3위 안이 아닐 때.
...
7. **범용이다.** 12 클러스터 안이면 어떤 주제든 후보다. 다른 사이트와의 역할 분담은 선정에 쓰지 않는다(2026-09-15 운영자).
...
| `sisterAbove` | sister-sites | **쓰지 않는다** (범용 결정) | 일반 경쟁 문서로 셈 |
...
- **기준어 = 실업급여** (30일 평균 = 100). 호출마다 포함해 호출 간 정규화 문제를 없앤다. 기준어를 고정해 날짜가 달라도 비교된다.
...
3. **열린 자리인데 우리 글이 10위 밖** (§1-5) → 진단 리포트: 타깃 쿼리가 제목 선두에 있나 · 1차 출처·날짜가 올해인가 · 우리 글끼리 같은 쿼리를 나눠 먹는가 · 1위 문서가 가진 구체 정보(표·사례·기한) 중 우리 글에 없는 것
...
2. 파이프라인 1회 실행으로 **머니룩 주제 후보 5건 이상**이 점수 순으로 큐와 보고서에 나오고, 후보마다 이유·조건이 붙는다. 12 클러스터 밖 주제나 도구·네이버 서비스 점령 쿼리가 "높음"으로 올라오면 실패.
...
| 3 | 축별 신규 글 10위 진입률 (어느 축이 먹히나) | 미측정 | 8축 모두 기준선 확보 |
| 4 | 도구·네이버 서비스 점령 쿼리에 신규 발행 | 미측정 | 0 |
...
| # | 항목 | 기본값 (Claude 권고) | 이유 |
|---|---|---|---|
| ~~1~~ | 주제 범위 | **결정됨(2026-09-15)**: 범용. 12 클러스터 안이면 전부 후보, 다른 사이트 역할 분담은 쓰지 않는다 | 운영자 |
| ~~4~~ | T1 물결에도 신규 일 1편 유지 | **결정됨(2026-09-15): 유지**, 넘치면 갱신 | 운영자 |
| ~~5~~ | [발행 지시] 버튼 = 운영자 승인 → Claude 가 `merge-approved` 부착 | **결정됨(2026-09-15): 허용**, 항목 단위·일 1편 이내. CLAUDE.md 개정은 §10-14 | 운영자 |
| 6 | 서치어드바이저(검색어·문서) + 애널리틱스 검색어 CSV 주 1회 투입 | 한다 (월요일) | 가중치 보정의 유일한 실측 |
| ~~7~~ | GitHub 시크릿 HUB 키 2개 등록 | **완료(2026-09-15)** — 이름 확인, 값은 첫 워크플로 실행 때 확인 | 운영자 |
| 8 | 웹문서 위치 눈 확인 하루 5건 | 한다 (1분) | API 로 못 재는 유일한 게이트 |
...
- 범용으로 넓힌 뒤 어느 축이 실제 유입을 내는지는 CSV 가 들어와야 안다. 축별 배분은 2주 뒤 정한다.
